...
std::string server::topic_command(request req, int fd)
{
	...
	else
	{
		if (_channels.find(req.args[0]) != _channels.end())
		{
			if (_channels[req.args[0]]->isMember(clnt))
			{
				if (req.args.size() == 1)
				{
					if ((_channels[req.args[0]]->get_topic()).empty())
						send_replay(clnt, prefix, "331", nick, req.args[0] + " " + RPL_NOTOPIC);
					else
						send_replay(clnt, prefix, "332", nick, req.args[0] + " :" + _channels[req.args[0]]->get_topic() + "\n");
				}
				...
				else
				{
					...
					send_replay(clnt, prefix, "TOPIC", req.args[0], ":" + _channels[req.args[0]]->get_topic() + "\n");
				}
			}
			else
			...
			send_replay(clnt, prefix, "403", nick, req.args[0] + " " + ERR_NOSUCHCHANNEL);
	}
	return "";
}