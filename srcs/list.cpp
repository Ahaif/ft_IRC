...
std::string server::list_command(request req, int fd)
{
	...
	else if (req.args.size() != 0 && req.args[0] != "")
	{
		...
		send_replay(clnt, prefix, "321", nick, RPL_LISTSTART);
		for (size_t i = 0; i < names.size(); i++)
		{
			...
			{
				message = it->first + " " + std::to_string(it->second->get_onlineUsers()) + " :";
				if (it->second->isPrivate() == true && it->second->isMember(clnt) == false)
					topic = "";
				else
					topic = it->second->get_topic();
				message += topic;
				send_replay(clnt, prefix, "322", nick, message);
			}
		}
		send_replay(clnt, prefix, "322", nick, RPL_LISTEND);
	}
	else
	{
